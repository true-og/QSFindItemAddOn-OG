...
public class LocationUtils {
    ...
    static {
        ...
        nonSuffocatingBlocks.addAll(new ArrayList<>(Arrays.asList(Material.values()).stream()
                .filter(p -> p.toString().toLowerCase().contains("_glass")).toList()));
        // Glass Panes
        nonSuffocatingBlocks.addAll(new ArrayList<>(Arrays.asList(Material.values()).stream()
        ...
        // Slabs
        nonSuffocatingBlocks.addAll(new ArrayList<>(Arrays.asList(Material.values()).stream()
                .filter(p -> p.toString().toLowerCase().contains("_slab")).toList()));
        // Walled Signs
        nonSuffocatingBlocks.addAll(new ArrayList<>(Arrays.asList(Material.values()).stream()
                .filter(p -> p.toString().toLowerCase().contains("_sign")).toList()));
        // Stairs
        nonSuffocatingBlocks.addAll(new ArrayList<>(Arrays.asList(Material.values()).stream()
                .filter(p -> p.toString().toLowerCase().contains("_stairs")).toList()));
        // MISC
    ...
    public static Location findSafeLocationAroundShop(Location shopLocation) {
        ...
        QSApi qsApi;
        if (FindItemAddOnOG.isQSReremakeInstalled()) {
        ...
        LoggerUtils.logDebugInfo("Rounded location: " + roundedShopLoc.getX() + ", " + roundedShopLoc.getY() + ", "
                + roundedShopLoc.getZ());
        ...
        for (Location loc_i : possibleSafeLocList) {
            ...
            if (loc_i.getBlock().getType().equals(qsApi.getShopSignMaterial())) {
                ...
                Location blockAbove = new Location(loc_i.getWorld(), loc_i.getBlockX(), loc_i.getBlockY() + 1,
                        loc_i.getBlockZ());
                LoggerUtils.logDebugInfo("Block above shop sign: " + blockAbove.getX() + ", " + blockAbove.getY() + ", "
                        + blockAbove.getZ());
                if (!isBlockSuffocating(blockAbove)) {
                    ...
                    for (int i = 1; i <= BELOW_SAFE_BLOCK_CHECK_LIMIT; i++) {
                        ...
                        blockBelow = new Location(loc_i.getWorld(), loc_i.getBlockX(), loc_i.getBlockY() - i,
                                loc_i.getBlockZ());
                        LoggerUtils.logDebugInfo("Block below shop sign: " + blockBelow.getBlock().getType() + " "
