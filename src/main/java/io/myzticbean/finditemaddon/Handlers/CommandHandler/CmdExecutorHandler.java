...
public class CmdExecutorHandler {
    ...
    public void handleShopSearch(String buySellSubCommand, CommandSender commandSender, String itemArg) {
        ...
        if (!(commandSender instanceof Player)) {
            ...
            if (player.hasPermission(PlayerPerms.FINDITEM_USE.value())) {
                ...
                boolean isBuying;
                if (StringUtils.isEmpty(FindItemAddOnOG.getConfigProvider().FIND_ITEM_TO_BUY_AUTOCOMPLETE)
                        || StringUtils.containsIgnoreCase(
                                FindItemAddOnOG.getConfigProvider().FIND_ITEM_TO_BUY_AUTOCOMPLETE, " "))
                {
                    ...
                    isBuying = buySellSubCommand
                            .equalsIgnoreCase(FindItemAddOnOG.getConfigProvider().FIND_ITEM_TO_BUY_AUTOCOMPLETE);
        ...
        if (!(commandSender instanceof Player)) {
            ...
            if (player.hasPermission(PlayerPerms.FINDITEM_HIDESHOP.value())) {
                ...
                if (FindItemAddOnOG.isQSReremakeInstalled()) {
                    ...
                    hideShop((Shop) FindItemAddOnOG.getQsApiInstance().findShopAtLocation(playerLookAtBlock), player);

                } else {

                    hideShop((com.ghostchu.quickshop.api.shop.Shop) FindItemAddOnOG.getQsApiInstance()
        ...
        if (!(commandSender instanceof Player)) {
            ...
            if (player.hasPermission(PlayerPerms.FINDITEM_HIDESHOP.value())) {
                ...
                if (FindItemAddOnOG.isQSReremakeInstalled()) {
                    ...
                    revealShop((Shop) FindItemAddOnOG.getQsApiInstance().findShopAtLocation(playerLookAtBlock), player);

                } else {

                    revealShop((com.ghostchu.quickshop.api.shop.Shop) FindItemAddOnOG.getQsApiInstance()
                            .findShopAtLocation(playerLookAtBlock), player);
     ...
     */
    /*
     * public void handleHiddenShopSavingToFile(CommandSender commandSender) { if
     ...
     */
    /*
     * public void handleHiddenShopLoadingFromFile(CommandSender commandSender) { if
     * (!(commandSender instanceof Player)) {
     * LoggerUtils.logInfo("This command can only be run from in game"); } else {
     * Player player = (Player) commandSender;
     * if(player.hasPermission(PlayerPerms.FINDITEM_ADMIN.toString())) {
    ...
    public void handlePluginReload(CommandSender commandSender) {
        ...
        if (!(commandSender instanceof Player)) {
            ...
            Player player = (Player) commandSender;
            if (player.hasPermission(PlayerPerms.FINDITEM_RELOAD.value())
                    || player.hasPermission(PlayerPerms.FINDITEM_ADMIN.value()))
            {
    ...
    public void handlePluginRestart(CommandSender commandSender) {
        ...
        if (!(commandSender instanceof Player)) {
            ...
            Player player = (Player) commandSender;
            if (player.hasPermission(PlayerPerms.FINDITEM_RESTART.value())
                    || player.hasPermission(PlayerPerms.FINDITEM_ADMIN.value()))
            {
    ...
    private void hideShop(org.maxgamer.quickshop.api.shop.Shop shop, Player player) {
        ...
        if (shop != null) {
            ...
            if (FindItemAddOnOG.getQsApiInstance().isShopOwnerCommandRunner(player, shop)) {
