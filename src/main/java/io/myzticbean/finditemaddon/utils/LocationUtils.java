...
public class LocationUtils {
	...
	public static Location findSafeLocationAroundShop(Location shopLocation, Player player) {
		...
		FindItemAddOn.logger("Rounded location: " + roundedShopLoc.getX() + ", " + roundedShopLoc.getY() + ", " + roundedShopLoc.getZ());
		...
		for(Location loc_i : possibleSafeLocList) {
			...
			if(loc_i.getBlock().getType().equals(FindItemAddOn.getQsApiInstance().getShopSignMaterial())) {
				FindItemAddOn.logger("Shop sign block found at " + loc_i.getX() + ", " + loc_i.getY() + ", " + loc_i.getZ());
				...
				Location blockAbove = new Location(
						...
						loc_i.getBlockY() + 1,
						loc_i.getBlockZ());
				FindItemAddOn.logger("Block above shop sign: "
				...
				if(!isBlockSuffocating(blockAbove)) {
					...
					FindItemAddOn.logger("Block above shop sign found not air. Block type: " + blockAbove.getBlock().getType());
					return null;
				}
			}
			else {
				FindItemAddOn.logger("Block not shop sign. Block type: " + loc_i.getBlock().getType());
			}
		}
