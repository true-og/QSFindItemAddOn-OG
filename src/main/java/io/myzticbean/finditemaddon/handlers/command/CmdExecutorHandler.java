...
public class CmdExecutorHandler {
    ...
    public void handleShopSearch(String buySellSubCommand, CommandSender commandSender, String itemArg) {
                ...
                .containsIgnoreCase(QSFindItemAddOnOG.getConfigProvider().FIND_ITEM_TO_BUY_AUTOCOMPLETE, " "))
        {
            ...
            isBuying = buySellSubCommand
                    .equalsIgnoreCase(QSFindItemAddOnOG.getConfigProvider().FIND_ITEM_TO_BUY_AUTOCOMPLETE);
                ...
                && !QSFindItemAddOnOG.getConfigProvider().FIND_ITEM_CMD_DISABLE_SEARCH_ALL_SHOPS)
        {
                    ...
                    && QSFindItemAddOnOG.getQsApiInstance().isQSShopCacheImplemented())
            {
                ...
                Bukkit.getScheduler().runTaskAsynchronously(QSFindItemAddOnOG.getInstance(), () -> {
                    ...
                    this.openShopMenu(player, searchResultList, true,
                ...
                List<FoundShopItemModel> searchResultList = QSFindItemAddOnOG.getQsApiInstance()
                        .fetchAllItemsFromAllShops(isBuying, player);
                this.openShopMenu(player, searchResultList, false,
                        QSFindItemAddOnOG.getConfigProvider().NO_SHOP_FOUND_MSG);
    ...
    private void openShopMenu(Player player, List<FoundShopItemModel> searchResultList, boolean synchronize,
            String errorMsg)
    {
        ...
        if (!searchResultList.isEmpty()) {
            ...
            if (synchronize) {
                ...
                Bukkit.getScheduler().runTask(QSFindItemAddOnOG.getInstance(), () -> {
                    ...
                    FoundShopsMenu menu = new FoundShopsMenu(QSFindItemAddOnOG.getPlayerMenuUtility(player),
                            searchResultList);
        ...
        if (commandSender instanceof Player player) {
            ...
            if (player.hasPermission(PlayerPermsEnum.FINDITEM_HIDESHOP.value())) {
                ...
                if (QSFindItemAddOnOG.isQSReremakeInstalled()) {
                    ...
                    hideReremakeShop((Shop) QSFindItemAddOnOG.getQsApiInstance().findShopAtLocation(playerLookAtBlock),
                            player);

                } else {

                    hideHikariShop((com.ghostchu.quickshop.api.shop.Shop) QSFindItemAddOnOG.getQsApiInstance()
                            .findShopAtLocation(playerLookAtBlock), player);
                ...
                UtilitiesOG.trueogMessage(player,
                        QSFindItemAddOnOG.getConfigProvider().PLUGIN_PREFIX + "&cNo permission!");
            ...
            if (player.hasPermission(PlayerPermsEnum.FINDITEM_HIDESHOP.value())) {
                ...
                if (playerLookAtBlock != null) {
                    ...
                    if (QSFindItemAddOnOG.isQSReremakeInstalled()) {
                        ...
                        revealShop((Shop) QSFindItemAddOnOG.getQsApiInstance().findShopAtLocation(playerLookAtBlock),
                                player);

                    } else {

                        revealShop((com.ghostchu.quickshop.api.shop.Shop) QSFindItemAddOnOG.getQsApiInstance()
                                .findShopAtLocation(playerLookAtBlock), player);
    ...
    public void handlePluginReload(CommandSender commandSender) {
        ...
        if (!(commandSender instanceof Player player)) {
                    ...
                    || player.hasPermission(PlayerPermsEnum.FINDITEM_ADMIN.value()))
            {
                ...
                QSFindItemAddOnOG.initConfigProvider();
                UtilitiesOG.trueogMessage(player,
                        QSFindItemAddOnOG.getConfigProvider().PLUGIN_PREFIX + "&aConfig reloaded!");
                ...
                if (allServerShops.isEmpty()) {
                    ...
                    UtilitiesOG.trueogMessage(player, QSFindItemAddOnOG.getConfigProvider().PLUGIN_PREFIX + "&aFound &e"
                            + allServerShops.size() + " &ashops on the server.");
    ...
    public void handlePluginRestart(CommandSender commandSender) {
        ...
        if (!(commandSender instanceof Player)) {
                    ...
                    || player.hasPermission(PlayerPermsEnum.FINDITEM_ADMIN.value()))
            {
                ...
                Bukkit.getPluginManager().enablePlugin(QSFindItemAddOnOG.getPlugin(QSFindItemAddOnOG.class));
                UtilitiesOG.trueogMessage(player,
                        QSFindItemAddOnOG.getConfigProvider().PLUGIN_PREFIX + "&aPlugin restarted!");
                List<?> allServerShops = QSFindItemAddOnOG.getQsApiInstance().getAllShops();
                if (allServerShops.size() == 0) {
