...
public class FoundShopsMenu extends PaginatedMenu {
    ...
    public TextComponent getMenuName() {
        if (!StringUtils.isEmpty(configProvider.SHOP_SEARCH_GUI_TITLE)) {
            return UtilitiesOG.trueogColorize(configProvider.SHOP_SEARCH_GUI_TITLE);
        } else {
    ...
    private void handleDirectShopTeleport(@NotNull Player player, List<String> locDataList) {
        ...
        if (player.getUniqueId().equals(shopOwner) && !PlayerPermsEnum.canPlayerTpToOwnShop(player)) {
            UtilitiesOG.trueogMessage(player, configProvider.PLUGIN_PREFIX + configProvider.SHOP_TP_NO_PERMISSION_MSG);
            return;
        }
        ...
        if (configProvider.CUSTOM_CMDS_RUN_ENABLED
            ...
            for (String cmd : configProvider.CUSTOM_CMDS_LIST) {
                Bukkit.dispatchCommand(
                        Bukkit.getConsoleSender(), replaceCustomCmdPlaceholders(cmd, player, shopLocation));
            }
        }
    }
    ...
    private @Nullable Location parseShopLocation(@NotNull List<String> locDataList) {
        if (locDataList.size() <= 1) return null;
    ...
    private void sendNoPermissionMessage(Player player) {
        if (!StringUtils.isEmpty(configProvider.SHOP_TP_NO_PERMISSION_MSG)) {
            UtilitiesOG.trueogMessage(
                    player, (configProvider.PLUGIN_PREFIX + configProvider.SHOP_TP_NO_PERMISSION_MSG));
        }
    }
    ...
    private void sendUnsafeAreaMessage(Player player) {
        if (!StringUtils.isEmpty(configProvider.UNSAFE_SHOP_AREA_MSG)) {
            UtilitiesOG.trueogMessage(player, (configProvider.PLUGIN_PREFIX + configProvider.UNSAFE_SHOP_AREA_MSG));
        }
    }
    ...
    private void applyTeleportDelay(Player player, Location locToTeleport) {
        ...
        if (!StringUtils.isEmpty(tpDelayMsg)) {
            UtilitiesOG.trueogMessage(
                    player, configProvider.PLUGIN_PREFIX + replaceDelayPlaceholder(tpDelayMsg, delay));
        }
        Bukkit.getScheduler()
    ...
    private void handleMenuClickForNavToNextPage(InventoryClickEvent event) {
        if (!((index + 1) >= super.playerMenuUtility.getPlayerShopSearchResult().size())) {
            ...
            if (!StringUtils.isEmpty(configProvider.SHOP_NAV_LAST_PAGE_ALERT_MSG)) {
                event.getWhoClicked()
                        .sendMessage(UtilitiesOG.trueogColorize(
    ...
    private void handleMenuClickForNavToPrevPage(InventoryClickEvent event) {
        if (page == 0) {
            if (!StringUtils.isEmpty(configProvider.SHOP_NAV_FIRST_PAGE_ALERT_MSG)) {
                event.getWhoClicked()
                        .sendMessage(UtilitiesOG.trueogColorize(
                                configProvider.PLUGIN_PREFIX + configProvider.SHOP_NAV_FIRST_PAGE_ALERT_MSG));
            }
        } else {
            page = page - 1;
            super.open(super.playerMenuUtility.getPlayerShopSearchResult());
    ...
    private @NotNull ItemStack createShopItem(@NotNull FoundShopItemModel foundShop) {
        ItemStack item =
                new ItemStack(foundShop.getItem().getType(), foundShop.getItem().getAmount());
        ...
        // Preserve custom model data
        ItemMeta originalMeta = foundShop.getItem().getItemMeta();
    ...
    private void addItemLore(List<Component> lore, FoundShopItemModel foundShop) {
        UUID ownerUUID = foundShop.getShopOwner();
        // TODO: Use player cache.
        OfflinePlayer owner = Bukkit.getOfflinePlayer(ownerUUID);
        String ownerName = owner.getName() != null ? owner.getName() : "Unknown";
        lore.add(UtilitiesOG.trueogExpand("<green>Owner: <white>" + ownerName));
        lore.add(UtilitiesOG.trueogExpand("<yellow>Price: <gold>" + foundShop.getShopPrice()));
        ...
        for (String raw : configProvider.SHOP_GUI_ITEM_LORE) {
            raw = raw.replace(ShopLorePlaceholdersEnum.NEAREST_WARP.value(), getNearestWarpInfo(foundShop));
            lore.add(UtilitiesOG.trueogExpand(raw));
        }
        ...
        if (configProvider.TP_PLAYER_DIRECTLY_TO_SHOP
            ...
            lore.add(UtilitiesOG.trueogExpand(configProvider.CLICK_TO_TELEPORT_MSG));
        }
    }
        ...
        switch (nearestWarpMode) {
            case 1:
                ...
                if (EssentialsXPlugin.isEnabled()) {
                    ...
                    return (nearestEWarp != null && !StringUtils.isEmpty(nearestEWarp))
                            ? nearestEWarp
            ...
            case 2:
                ...
                if (PlayerWarpsPlugin.getIsEnabled()) {
                    Warp nearestPlayerWarp =
                            PlayerWarpsUtil.findNearestWarp(foundShop.getShopLocation(), foundShop.getShopOwner());
                    return (nearestPlayerWarp != null)
                            ? nearestPlayerWarp.getWarpName()
                            : configProvider.NO_WARP_NEAR_SHOP_ERROR_MSG;
                }
                break;
            case 3:
                ...
                if (WGPlugin.isEnabled()) {
                    ...
                    return (nearestWGRegion != null && !StringUtils.isEmpty(nearestWGRegion))
                            ? nearestWGRegion
            ...
            case 4:
                ...
                if (ResidencePlugin.isEnabled()) {
                    ClaimedResidence nearestResidence =
                            ResidenceUtils.findNearestResidence(foundShop.getShopLocation());
                    return (nearestResidence != null)
                            ? ResidenceUtils.getResidenceName(nearestResidence)
    ...
    private void setLocationData(ItemMeta meta, FoundShopItemModel foundShop) {
        ...
        if (configProvider.TP_PLAYER_DIRECTLY_TO_SHOP) {
            ...
            locData = String.format(
                    "%s,%d,%d,%d",
                    shopLoc.getWorld().getName(), shopLoc.getBlockX(), shopLoc.getBlockY(), shopLoc.getBlockZ());
        } else if (configProvider.TP_PLAYER_TO_NEAREST_WARP) {
    ...
    private @NotNull String replaceLorePlaceholders(String text, @NotNull FoundShopItemModel shop) {
        ...
        text = text.replace(
                ShopLorePlaceholdersEnum.SHOP_PER_ITEM_QTY.value(),
        ...
        text = text.replace(
                ShopLorePlaceholdersEnum.SHOP_WORLD.value(),
                Objects.requireNonNull(shop.getShopLocation().getWorld()).getName());

        // Added in v2.0
        text = text.replace(
                ShopLorePlaceholdersEnum.SHOP_VISITS.value(),
    ...
    private String replaceCustomCmdPlaceholders(String cmd, Player player, Location shopLoc) {
        return cmd.replace(CustomCmdPlaceholdersEnum.PLAYER_NAME.value(), player.getName())
