...
public class FindItemCmdHikariImpl implements CommandHandler<Player> {
	...
	public void onCommand(Player commandSender, @NotNull String label, @NotNull String[] args) {
		if (args.length == 0) {
			UtilitiesOG.trueogMessage(commandSender, (FindItemAddOn.getConfigProvider().PLUGIN_PREFIX + "&cIncorrect usage!"));
			...
			if (commandSender.hasPermission(PlayerPermsEnum.FINDITEM_HIDESHOP.value())
				...
				UtilitiesOG.trueogMessage(commandSender, FindItemAddOn.getConfigProvider().PLUGIN_PREFIX + "&cYou don't have permission to use that!");
			}
		} else {
